...
The current Streamlit dashboard is implemented in `env/Scripts/Data_Insights.py` and connects to a PostgreSQL database for live transaction and user metrics.
...
```bash
streamlit run env/Scripts/Data_Insights.py
```
For example - streamlit run "D:\GITHUB REPO\data-insights-engine\env\Scripts\Data_Insights.py"
...
- SQL query helpers are defined in `env/Scripts/Data_Insights.py` and currently connect to the `smart_pay` PostgreSQL database.
